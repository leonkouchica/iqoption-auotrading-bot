"""
Risk Manager — handles all risk management logic.

Tracks daily stats, enforces daily limits, calculates position sizes,
and manages consecutive-loss cooloffs.
"""
...
class RiskManager:
    ...
    def __init__(self, config: TradingConfig):
        ...
        # Cool-off tracking — single mechanism (timestamp-based)
        self.cooloff_until = 0  # Unix timestamp when cool-off ends
    ...
    def update_balance(self, new_balance: float):
        """Update current balance and track peak. Always syncs to API value."""
    ...
    def can_trade(self) -> Tuple[bool, str]:
        ...
        # ── Cooloff (timestamp-based — sole mechanism for consecutive losses) ──
        if time.time() < self.cooloff_until:
            remaining = int(self.cooloff_until - time.time())
            return False, f"Cool-off active: {remaining}s remaining"
        ...
        if current_hour < self.config.trading_start_hour or current_hour > self.config.trading_end_hour:
            return False, (
                f"Outside trading hours: {current_hour}:00 "
                f"(trading hours: {self.config.trading_start_hour}:00–{self.config.trading_end_hour}:00)"
            )

        return True, "Ready to trade"

    def calculate_position_size(self, scale: float = 1.0) -> float:
        """
        Calculate position size based on risk percentage, then apply
        an optional scale factor (e.g. 0.8 for STRONG, 0.6 for reversal signals).
    ...
    def record_trade(self, pnl: float):
        ...
        NOTE: does NOT touch self.current_balance — balance is synced from
        the API via update_balance() to avoid double-counting.
        """
        self.daily_trades += 1
        self.daily_pnl    += pnl
        ...
        if pnl > 0:
            ...
            # Trigger cooloff if consecutive-loss threshold reached
            if self.consecutive_losses >= self.config.max_consecutive_losses:
                self.cooloff_until = time.time() + (self.config.cooloff_minutes * 60)
                logger.warning(
                    f"⏸️  COOL-OFF TRIGGERED! {self.config.cooloff_minutes} min pause "
    ...
    def print_status(self):
        ...
        if self.cooloff_until > time.time():
            remaining = int(self.cooloff_until - time.time())
            logger.info(f"⏸️  Cool-off:       {remaining}s remaining")
        logger.info(f"{'='*50}\n")
