...
def get_trade_decision(candle: Dict) -> Direction:
    ...
    Returns:
        CandleColor enum value
    """
    if candle["close"] > candle["open"]:
        logger.info(f"Signal -> CALL")
        return Direction.CALL
    elif candle["close"] < candle["open"]:
        logger.info(f"Signal -> PUT")
        return Direction.PUT
    logger.info(f"Signal -> INDECISION")
...
def print_trade_outcome(trade_record: TradeRecord):
    ...
    # P&L with color and + sign for profits
    if trade_record.profit_loss > 0:
    ...
    # Add a cute progress bar for profit/loss visualization
    if trade_record.profit_loss != 0:
...
def is_current_seconds_between_zero_and_two(timestamp):
    """
    Check if the seconds portion of a timestamp is between 0 and 2 (inclusive).
    
    Args:
        timestamp: Unix timestamp in milliseconds (like 1775996973735)
    
    Returns:
        bool: True if seconds are 0, 1, or 2, False otherwise
    """
    ...
    # Check if seconds is between 0 and 2 (inclusive)
    return 0 <= seconds <= 29
...
# # Usage example with your timestamp
# server_time = 1775996973735
# result = is_current_seconds_between_zero_and_two(server_time)
# print(f"Result: {result}")
# print(f"Seconds: {datetime.fromtimestamp(server_time / 1000).second}")